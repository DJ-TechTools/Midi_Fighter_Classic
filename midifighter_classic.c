...
//   0.22: 4/11/12 Added new jumptoboot code to fix issue with new bootloader
...
void Midifighter_Task(void)
{
    ...
    if (g_exp_analog_read) {
        ...
        static uint16_t adc_value[NUM_ANALOG_MULTIPLEXED];
        uint8_t max_analog = 0;

        if (g_multiplexer_enable) {
            max_analog = NUM_ANALOG_MULTIPLEXED;

            if (g_exp_analog_read & 0x1) {adc_value[0] = exp_adc_read(0);}
            if (g_exp_analog_read & 0x2) {adc_value[1] = exp_adc_read(1);}
            if (g_exp_analog_read & 0x4) {adc_value[2] = exp_adc_read(2);}

            if (g_exp_analog_read & 0x8) {
